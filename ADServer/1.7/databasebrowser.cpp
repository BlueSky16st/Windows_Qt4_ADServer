...
bool DatabaseBrowser::m_isConnected;
	...
	if(m_db.databaseName().isEmpty())
	{
		...
		while(!in.atEnd())
		{
			...
			if(text.startsWith("HostName"))
				m_db.setHostName(text.mid(text.indexOf('=') + 1).simplified());
			if(text.startsWith("DatabaseName"))
				m_db.setDatabaseName(text.mid(text.indexOf('=') + 1).simplified());
			if(text.startsWith("UserName"))
				m_db.setUserName(text.mid(text.indexOf('=') + 1).simplified());
			if(text.startsWith("Password"))
				m_db.setPassword(text.mid(text.indexOf('=') + 1).simplified());
		}
		file.close();
	}
 ...
 *  函数名：isConnected
 *  函数类型：公有函数，静态函数
 *  时间：2016/10/07 21:23:26
 *  功能：检查数据库是否已连接。
 *  
 *  返回值：如果数据库已连接，返回true，否则返回false。
 */
bool DatabaseBrowser::isConnected()
{
...
void DatabaseBrowser::deleteScene()
{
	...
	if(QMessageBox::information(this, tr("编辑数据库"),
								tr("确认删除该广告节目单？"),
