...
void MainWindow::changeEvent(QEvent * event)
{
	if(event->type() == QEvent::LanguageChange)
	{
		...
		editModeRadioButton->setText(tr("编辑器模式(&M)"));
		databaseModeRadioButton->setText(tr("数据库模式(&D)"));
...
void MainWindow::editFile()
{
	...
	if(databaseModeRadioButton->isChecked() 
			&& !connectOrDisconnectDatabaseButton->isChecked())
	{
		QMessageBox::information(this, tr("广告服务器"), tr("数据库未连接。"));
		return;
	}
	
	XMLEditDialog dlg(m_fileList.at(index), 
					  font(), 
					  editModeRadioButton->isChecked(), 
 ...
 *  函数名：setMode
 *	函数类型：私有槽
 *  时间：2016/9/27 20:11:16
 *  功能：设置当前模式为数据库模式或编辑器模式，从而显示或隐藏连接或断开数据库按钮。
 * 
 */
	...
	else
	{
		if(connectOrDisconnectDatabaseButton->isChecked())
		{
			QMessageBox::StandardButton btn =  
					QMessageBox::information(this,
											 tr("广告服务器"),
											 tr("数据库已连接，切换模式后将会断开数据库连接，是否继续？"),
											 QMessageBox::Yes | QMessageBox::No);
			
			if(btn == QMessageBox::No)
			{
				databaseModeRadioButton->setChecked(true);
				return;
			}
		}
		connectOrDisconnectDatabase(false);
	}
	
	connectOrDisconnectDatabaseButton->setVisible(bo);
	connectDatabaseWidget->setVisible(bo);
...
void MainWindow::initConnect()
{
	...
	connect(fileListTreeWidget, SIGNAL(currentItemChanged(QTreeWidgetItem*,QTreeWidgetItem*)),
			this, SLOT(fileBrowse()));
	connect(editModeRadioButton, SIGNAL(toggled(bool)),
			this, SLOT(setMode()));
	connect(databaseModeRadioButton, SIGNAL(toggled(bool)),
			this, SLOT(setMode()));
...
void MainWindow::readSettings()
{
	...
	editModeRadioButton->setChecked(settings.value("isEditMode", true).toBool());
	databaseModeRadioButton->setChecked(!settings.value("isEditMode", true).toBool());
	connectDatabaseWidget->setVisible(!settings.value("isEditMode", true).toBool());
...
void MainWindow::saveSettings()
{
	...
	settings.setValue("isEditMode", editModeRadioButton->isChecked());
