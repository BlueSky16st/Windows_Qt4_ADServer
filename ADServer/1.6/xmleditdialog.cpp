 ...
 * 参数1：当前要做操作的文件。
 * 参数2：程序字体。
 * 参数3：编辑器模式，默认为编辑器模式，false为数据库模式
 * 参数4：是否为中文，默认为中文。
 * 参数5：父对象，默认为0。
 * 设置程序语言、预加载程序语言翻译文件、设置窗口UI、
							 ...
							 QWidget * parent /* = 0 */) : QDialog(parent)
{
	...
	adTypeWidget->setEnabled(editMode);
	tabWidget->setVisible(false);
	tabWidget->setEnabled(editMode);
...
void XMLEditDialog::createNewAD()
{
	...
	else if(item->text(0) == tr("双击添加新的广告节目单"))
	{
		...
		item->setText(0, m_editMode ? tr("新节目单") : newScene->name());
		item->setForeground(0, QBrush(Qt::black));
		adNameLineEdit->setText(m_editMode ? tr("新节目单") : newScene->name());
		if(!m_editMode)
		{
			beginTimeEdit->setTime(newScene->begin());
			endTimeEdit->setTime(newScene->end());
			intervalSpinBox->setValue(newScene->interval());
		}
 ...
 * 私有槽
 * 参数1：当前鼠标位置
 * 在[广告列表]中使用鼠标右键，显示上下文菜单。
 * 鼠标位置不同会使菜单选项不同。
 * 如果在项上鼠标右键，则会有关于删除项的相关操作。
 * 如果在非项上鼠标右键，则会有关于添加广告节目单的相关操作。
 * 
*/
void XMLEditDialog::showContextMenu(QPoint pos)
{
	...
	if(item)
	{
		...
		else if(item->text(0) != tr("双击添加新的广告") 
